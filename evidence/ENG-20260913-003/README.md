...
| R1 evidence HEAD | `45fb2722e107e98409d20f5692ee42e62d1eb104` |
| R2 code HEAD | `19fea5abd545bbba6506a6c693f65568ff4381a5` |
| Status | DRAFT — R2 in review; no merge; no production writes |
...
## R2 (ChatGPT MERGE NO-GO remaining P1)

- Agent Check `PASS` requires READY=`true` **and** GitHub Actions variable `NPM_TRUSTED_PUBLISHING_CONFIG_SHA256` (outside the repo) == computed `CONTROL_PLANE_CONFIG_SHA256` **and** committed fingerprint integrity **and** GitHub OK.
- Missing / unreadable / mismatch approved hash → `LIVE_AUDIT_REQUIRED`. Check paths never invent or write the hash.
- Regression: READY=true, live-approved=A, committed/computed=B → `LIVE_AUDIT_REQUIRED` (PASS forbidden).
- Production `--check-agent` after R2 (`check-agent-r2.log`): verdict=`BLOCKED`; env secrets 403; approved config sha **unreadable** (`LIVE_AUDIT_REQUIRED`); fingerprint `CACHED_OK`; npm live **NOT QUERIED**; writes=**0**. Not a fake PASS.
- Local R2 gates: `GATES-R2.md` / `quality-gates-r2.log` — all PASS.
...
| `check-agent-production.log` | Read-only production `--check-agent` (first tip) |
| `GATES-R1.md` / `quality-gates-r1.log` / `check-agent-r1.log` | R1 local gates + production `--check-agent` |
| `GATES-R2.md` / `quality-gates-r2.log` / `check-agent-r2.log` | R2 local gates + production `--check-agent` |
