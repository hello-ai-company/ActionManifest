...
> **Release status:** `0.9.0-rc.0` is on npm (no git tag / GitHub Release).
...
>   binding, requires Node >= 22. Release Check gates a native smoke (2/2)
>   on the canonical tarball; `pnpm xberg:integration` is the local opt-in.
